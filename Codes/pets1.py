...
def collect_rollout(env,policy):
    ...
    T=env._max_episode_steps #2 #task horizon
...
class PE(nn.Module):
    ...
    def __init__(self, B, in_size, n, h, out_size,device):
        ...
        self.max_logvar = nn.parameter.Parameter(0.5 * torch.ones(1, out_size // 2, device =self.device, dtype=torch.float32))
        self.min_logvar = nn.parameter.Parameter(-10.0 * torch.ones(1, out_size // 2, device =self.device, dtype=torch.float32))
    ...
    def initialize(self,in_size,out_size):
        ...
        w = nn.parameter.Parameter(torch.rand(self.B,in_size,out_size,device=self.device,dtype=torch.float32))
        truncated_normal(w.data, mean=mu, std=std)
        b=nn.parameter.Parameter(torch.rand(self.B, 1, out_size,device=self.device,dtype=torch.float32))
        b.data.fill_(0.0)
    ...
    def fit_input_stats(self, inputs):
        ...
        self.mu = nn.parameter.Parameter(torch.zeros(self.in_size), requires_grad=False)
        self.sigma = nn.parameter.Parameter(torch.zeros(self.in_size), requires_grad=False)
...
class MPC:
    ...
    def train(self,rollout,b): #Train the policy with rollouts
        ...
        for _ in range(epochs): #for each epoch
            for batch_num in range(num_batches): # for each batch 
                ...
                loss += 0.01 * (self.model.max_logvar.sum() - self.model.min_logvar.sum()) # a constant
                loss += self.model.compute_decays() #returns decays.sum() #decays[layer] = decay_coeffs[layer]*MSE(w[layer]) #decay_coeffs=[input=0.0001, h=0.00025, output=0.0005]
    ...
    def CEM(self,obs): #MPC's optimizer: an action sequence optimizer
        ...
        while (t < self.opt_max_iters) and np.max(var) > epsilon:
            ...
            #1- generate sols
            samples = X.rvs(size=[self.pop_size, sol_dim]) * np.sqrt(constrained_var) + mean #sample from destanderdized distributiion
