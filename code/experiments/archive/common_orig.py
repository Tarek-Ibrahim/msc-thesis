...
class Discriminator(object): #basically: a binary classifier  #TIP: reward of discriminator (score) is in the interval [0,1] -> for the same env, starts high then reduces over time as agent gets better at that env, then the randomized parameters of the env are encouraged to vary to make the disc reward high again
...
class DiagGaussian(tf.keras.Model):
    def __init__(self, num_inputs, num_outputs):
        ...
        self.fc_mean=tf.keras.layers.Dense(num_outputs,kernel_initializer=tf.keras.initializers.Orthogonal(gain=1.0))

        # self.logstd = tf.Variable(tf.zeros(num_outputs,1)) 
    ...
    def call(self, x):
        ...
        # return FixedNormal(action_mean, tf.exp(self.logstd))
...
class SVPG:
    ...
    def compute_returns(self, next_value, rewards, masks, klds):
        ...
        for step in reversed(range(len(rewards))):
            ...
            return_ = self.gamma * masks[step] * return_ + (rewards[step] - self.kld_coeff * klds[step])
    ...
    def step2(self):
        ...
        for i in range(self.n_particles):
            ...
            for t in range(self.T_svpg):
                ...
                with tf.GradientTape(persistent=True) as tape:
                    action, value = self.select_action(i, current_sim_params)
                    # state = tf.expand_dims(tf.convert_to_tensor(current_sim_params,dtype=tf.float32),0)
                    ...
                    # # self.values[i][t] = value
                    self.values[i].append(value)
                # gradients_p = tape.gradient(self.particles[i].saved_log_probs[t], self.particles[i].trainable_variables)
        ...
        for i in range(self.n_particles):
            ...
            for j, tape in enumerate(self.tapes[i]):
                with tape:
                    ...
                    # policy_grad_particle.append(self.particles[i].saved_log_probs[j] * tf.stop_gradient(advantage))
                ...
                # grads_c.append(tape.gradient(advantage, self.particles[i].trainable_variables))
                # grads_p.append(tape.gradient(p_grad_particle, self.particles[i].trainable_variables))
                    
            # critic_loss = 0.5 * tf.reduce_mean(tf.square(tf.concat(advantages,0)))
            ...
            # gradients=[gc_var+gp_var for gc_var,gp_var in zip(gradients_c,gradients_p)]
