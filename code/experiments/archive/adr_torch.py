...
lr_agent=0.01 #0.001 #0.01 #0.001 #learning rate
h1_agent=64 #400 #64 #400
h2_agent=64 #300 #64 #300
gamma_agent=0.99 #discount factor
T_agent_init=1000 #1000 #number of timesteps before any updates
b_agent=100 #1000 #100 #1000 #batch size
...
class Discriminator(object):
    ...
    def train(self, ref_traj, rand_traj, eps):
        ...
        for _ in range(eps):
            ...
            disc_loss = self.disc_loss_func(g_o, torch.ones((len(rand_batch), 1), device=device)) + self.disc_loss_func(e_o,torch.zeros((len(ref_batch), 1), device=device)) #!!!: are we telling the discriminator that the rand_batch transitions SHOULD be different than the ref batch transitions in training? if the rand env has parameters close to the ref env or if the two batches have similar transitions (isn't that what is supposed to happen when the agent gets good at a particular env configuration), how does that affect the training of the discriminator? 
            #???: at the start of the training, the discriminator is rubbish anyway in discriminating between the rand and ref envs, and that will also contribute to the uniform exploration of the sampling space
...
h_disc=32 #128 #32 #128
lr_disc=0.02 #0.0002 #0.02 #0.002
b_disc=128
train_disc=True
...
class SVPG:
    ...
    def compute_kernel2(self, X):
        ...
        pairwise_dists = squareform(pdist(X_np))
        ...
        # Compute RBF Kernel
        # k = torch.exp(-torch.from_numpy(pairwise_dists**2).to(device).float() / h**2 / 2)
        k = torch.exp(-torch.from_numpy(pairwise_dists**2).to(device).float() / h)
    ...
    def train(self, simulator_rewards):
        ...
        for i in range(self.n_particles):
            ...
            actor_params=list(self.particles[i].actor.parameters()) + list(self.particles[i].dist.parameters())
            vec_param, vec_policy_grad = parameters_to_vector(actor_params, both=True)
        ...
        for i in range(self.n_particles):
            actor_params=list(self.particles[i].actor.parameters()) + list(self.particles[i].dist.parameters())
            vector_to_parameters(grad_theta[i], actor_params, grad=True)
            # self.optimizers[i].step()
...
T_svpg=2 #5 #2 #5 #length of one svpg particle rollout
lr_svpg=0.03 #0.0003 #0.03 #0.0003
gamma_svpg=0.99
h_svpg=16 #100 #16 #100
...
if __name__ == '__main__':
    ...
    t_agent=0
    T_agent=int(1e6) #5000 #int(1e6) #max agent timesteps
    ...
    while t_agent < T_agent:
        ...
        for t in range(T_svpg):
            ...
            rewards_tr_ep.append(rewards_agent.mean())
        ...
        #log progress
        plot_tr_rewards_mean.append(np.array(rewards_tr_ep).mean())
        plot_disc_rewards_mean.append(rewards_disc.mean())
        if t_agent % 1 == 0:
            log_msg="Rewards Agent: {:.2f}, Rewards Disc: {:.2f}".format(np.array(rewards_tr_ep).mean(),rewards_disc.mean())
            if verbose:
