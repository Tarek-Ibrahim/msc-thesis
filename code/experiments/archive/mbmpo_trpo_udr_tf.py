...
def collect_rollout_batch(envs, ds, da, policy, T, b, n_workers, queue, params=None,add_noise=False,noise_scale=0.1, initial=False,models=None,RB=None,vec_model=None): # a batch of rollouts
    ...
    while (not all(dones)) or (not queue.empty()):
        ...
        if models is not None:
            ...
            s_dash, r, dones, rollout_idxs_new, _ = vec_model.step(models,inputs)
        else:
...
class DynamicsModelWrapper(object):
    ...
    def __init__(self, ds, da, T_env, reward_func, H_max):
        super().__init__()
    ...
    def step(self,model,inputs):
        ...
        s_dash= (s+delta_s_dash).numpy()
        r=self.reward_func(s,a,s_dash)
...
class SubprocVecModel(object):
    ...
    def reset(self):
        ...
        rollouts_idxs = [parent_conn.recv() for parent_conn in self.parent_conns]
        return rollouts_idxs
        ...
        while True:
            ...
            if func == 'step':
                if self.done:
                    ...
                    state, reward, done, info = self.model.step(args)
...
def make_model(ds, da, T_env, reward_func, H_max, seed=None, rank=None):
    def _make_model():
        vec_model = DynamicsModelWrapper(ds, da, T_env, reward_func, H_max)
        return vec_model
    return _make_model


def make_vec_models(T_env, reward_func, H_max, seed, n_workers, ds, da, queue_model, lock_model):
    models=[make_model(ds, da, T_env, reward_func, H_max,seed,rank) for rank in range(n_workers)]
...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        vec_model = make_vec_models(T_env, reward_func, H_max, seed, n_workers, ds, da, queue_model, lock_model)
        ...
        for episode in episodes:
            ...
            for model in models:
                ...
                D, _= collect_rollout_batch(env, ds, da, policy, T_env, b, n_workers, queue_model, models=models,vec_model=vec_model)
                Ds.append(D)
                ...
                D_dash,_= collect_rollout_batch(env, ds, da, policy, T_env, b, n_workers, queue_model, params=theta_dash,models=models, vec_model=vec_model)
