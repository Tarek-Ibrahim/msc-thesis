        ...
        for i in range(self.n_particles):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                for t in range(self.T_svpg):
                    ...
                    # action = self._process_action(action) #???: why? --> this gives a direction #???: in case of discrete, when else?
                    action = np.array(np.clip(action, -1, 1)) #???: why not combine this and following line to scale action to region [-delta_max, delta_max]
                    clipped_action = action * self.delta_max #???: is this supposed to be a scaling or limiting factor?
                    next_params = np.clip(current_sim_params + clipped_action, 0, 1) #step
