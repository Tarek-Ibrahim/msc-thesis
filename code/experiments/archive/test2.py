...
#TODO: clean up and combine with test1.py
...
policies=[policy_trpo_adr,policy_trpo_udr]  
...
if alg == "trpo":
    ...
    filenames=["trpo_udr"]
    labels=["TRPO + UDR (absolute)","TRPO + UDR (running)"]
elif alg == "ppo":
...
for i, file_name in enumerate(filenames):
    ...
    policies[i+1].load_weights(f"saved_models/model_running_{common_name}")
    if "adr" in file_name:
...
for i, policy in enumerate(policies):
    ...
    for j, rand_value in enumerate(rand_range):
        ...
        for test_ep in range(test_eps):
            ...
            if False: # and "maml" in filenames[i]:
            ...
            while not done:
                ...
                if False: # and "maml" in filenames[i]:
                    ...
                    a=tf.squeeze(dist.sample()).numpy()
                elif True: #"trpo" in filenames[i] or "ppo" in filenames[i]:
                    dist=policy(state)
                    a=tf.squeeze(dist.sample()).numpy()
                else:
