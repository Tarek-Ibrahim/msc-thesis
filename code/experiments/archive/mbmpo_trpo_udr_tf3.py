...
def collect_rollout_batch(envs, ds, da, policy, T, b, n_workers, queue, params=None,add_noise=False,noise_scale=0.1, initial=False,RB=None,vec_model=None,state_dict=None): # a batch of rollouts
    ...
    while (not all(dones)) or (not queue.empty()):
        ...
        if state_dict is not None:
            s_dash, r, dones, rollout_idxs_new, _ = vec_model.step(state_dicts,s,a)
        else:
...
class DynamicsModel(tf.keras.Model):
    ...
    def __init__(self, ds, da, n, h, epochs, lr, b_train,lb,ub):
        super().__init__()
    ...
    def normalize_action(self, action):
        ...
        action = self.lb + 0.5 * (action + 1.0) * (self.ub - self.lb)
        return action
...
class DynamicsModelWrapper(object):
    ...
    def __init__(self, ds, da, T_env, reward_func, H_max,n,h,epochs,lr,b_train,env_name,lb,ub):
        super().__init__()
    ...
    def step(self,state_dict,s,a):
        ...
        model=DynamicsModel(self.ds,self.da,self.n,self.h,self.epochs,self.lr,self.b_train,self.lb,self.ub)
        model.set_weights(state_dict)
        
        a=model.normalize_action(a)
...
def make_model(ds, da, T_env, reward_func, H_max, n,h,epochs,lr,b_train,env_name,lb,ub,seed=None, rank=None):
    def _make_model():
        vec_model = DynamicsModelWrapper(ds, da, T_env, reward_func, H_max, n,h,epochs,lr,b_train,env_name,lb,ub)
        return vec_model
    return _make_model


def make_vec_models(T_env, reward_func, H_max, seed, n_workers, ds, da, n,h,epochs,lr,b_train,env_name,lb,ub, queue_model, lock_model):
    models=[make_model(ds, da, T_env, reward_func, H_max,n,h,epochs,lr,b_train,env_name,lb,ub,seed,rank) for rank in range(n_workers)]
...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        models = [DynamicsModel(ds,da,n,h_model,epochs,lr,b_model,lb,ub) for _ in range(B)]
        vec_model = make_vec_models(T_env, reward_func, H_max, seed, n_workers, ds, da,n,h_model,epochs,lr,b_model,env_name,lb,ub,queue_model, lock_model)
        ...
        for episode in episodes:
            ...
            #randomize rand envs #!!!: this only works if transitions within the same rollout is collected in the same environment (i.e. by the same [env] worker) #that's why we choose: n_particles=b(rollout batch size)=n_workers/n_envs #TODO: ensure this happens in a better way
            ...
            for model in models:
                ...
                #collect pre-adaptation rollout batch in rand envs
                D, _= collect_rollout_batch(env, ds, da, policy, T_env, b, n_workers, queue_model,vec_model=vec_model,state_dict=state_dict)
                Ds.append(D)
                ...
                D_dash,_= collect_rollout_batch(env, ds, da, policy, T_env, b, n_workers, queue_model, params=theta_dash, vec_model=vec_model,state_dict=state_dict)
