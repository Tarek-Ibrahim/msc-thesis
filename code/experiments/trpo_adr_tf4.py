...
import yaml
from common4 import set_seed, progress , parameters_to_vector, PolicyNetwork, ValueNetwork, surrogate_loss, HVP, conjugate_gradients, line_search, SVPG, Discriminator, collect_rollout_batch, make_vec_envs
...
if __name__ == '__main__':
    ...
    file_name="trpo_adr_cont4" #os.path.basename(__file__).split(".")[0]
    ...
    for seed in seeds:
        ...
        for episode in episodes:
            ...
            simulation_instances = svpg.step3() if episode >= T_svpg_init else -1 * np.ones((n_particles,dr))
            
            #create empty storages
            rewards_tr_ep = []
            D_dashes, D_dashes_ref, D_dashes_rand=[], [], []
            rewards_disc = np.zeros(simulation_instances.shape[:1])
            rewards_disc_raw = np.zeros(simulation_instances.shape[:1])
            scores_disc=np.zeros(simulation_instances.shape[:1])
            ...
            for _ in range(T_svpg):
                ...
                env_rand.randomize(simulation_instances)
                # env_rand.randomize([['default']*dr]*n_particles)
            ...
            #ADR updates
            for ref_traj, rand_traj in zip(D_dashes_ref,D_dashes_rand):
                ...
                for i in range(n_particles):
                    ...
                    r_disc, score_disc, log_score = discriminator.calculate_rewards(rand_traj[i])
                    rewards_disc[i]= r_disc
                    rewards_disc_raw[i]=log_score
                    scores_disc[i]=score_disc
            ...
            # plot_disc_log_rewards.append(rewards_disc.sum(-1).mean())
            # plot_disc_log_score.append(rewards_disc_raw.sum(-1).mean())
            
            #update svpg particles' params (ie. train their policies)
            if episode >= T_svpg_init:
                svpg.train3(rewards_disc)
        ...
        df = pd.DataFrame(list(zip(plot_tr_rewards_mean,
                          ...
                          columns =['Rewards_Tr_Mean', 'Rewards_Tr_Std', 'Rewards_Eval_Mean', 'Rewards_Eval_Std', 'Score_Disc_Mean', 'Score_Disc_Std', 'Total_Timesteps'])
        ...
        plt.figure(figsize=(16,8))
        plt.grid(1)
        plt.plot(plot_disc_rewards_mean,label='score')
        # plt.plot(plot_disc_log_rewards,label='reward (with pz)')
        # plt.plot(plot_disc_log_score,label='reward (without pz)')
