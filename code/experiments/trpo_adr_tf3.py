...
import yaml
from common3 import set_seed, progress , parameters_to_vector, PolicyNetwork, ValueNetwork, surrogate_loss, HVP, conjugate_gradients, line_search, SVPG, Discriminator, collect_rollout_batch, make_vec_envs
...
if __name__ == '__main__':
    ...
    T_svpg_init=int(1e6) #config["T_svpg_init"] 
    ...
    #general
    tr_eps=50 #config["tr_eps"]
    file_name="trpo_adr_cont3" #os.path.basename(__file__).split(".")[0]
    ...
    for seed in seeds:
        ...
        for episode in episodes:
            ...
            simulation_instances = svpg.step2() if episode >= T_svpg_init else -1 * np.ones((n_particles,T_svpg,dr))
            
            #create empty storages
            rewards_tr_ep = []
            D_dashes, D_dashes_ref, D_dashes_rand=[], [], []
            rewards_disc = np.zeros(simulation_instances.shape[:2])
            rewards_disc_raw = np.zeros(simulation_instances.shape[:2])
            ...
            for t, (ref_traj, rand_traj) in enumerate(zip(D_dashes_ref,D_dashes_rand)):
                ...
                for i in range(n_particles):
                    ...
                    rewards_disc_raw[i][t]=log_score
        ...
        df = pd.DataFrame(list(zip(plot_tr_rewards_mean,
                          ...
                          columns =['Rewards_Tr_Mean', 'Rewards_Tr_Std', 'Rewards_Eval_Mean', 'Rewards_Eval_Std', 'Score_Disc_Mean', 'Score_Disc_Std', 'Total_Timesteps','Rewards_Disc_Full','Rewards_Disc_Raw'])
