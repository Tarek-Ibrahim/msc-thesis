...
if __name__ == '__main__':
    ...
    parser.add_argument("--seed", "-S", type=int, default=10, help="Seed Value")
    ...
    in_size=ds
    out_size=da
    policy = PolicyNetwork(in_size,h,out_size).to(device) #dynamics model
    if args.agent_alg=="ppo": optimizer = Adam(policy.parameters(),lr=lr)
    if args.load_policy:
        policy.load_state_dict(torch.load(f"{models_dir}model{common_name}"+".pt")) #TODO: choose seed to load
        policy.eval()
    ...
    for episode in episodes:
        ...
        for t_rollout in range(T_rollout):
            ...
            if args.maml:
                ...
                Ds.append(D)
                _, _, rewards,_ = D
                rewards_tr_ep.append(rewards.cpu().numpy())
                t_agent += rewards.numel()
            ...
            t_agent += rewards.numel()
            t_eval += rewards.numel()
            if args.maml:
                rewards_val_ep.append(rewards.cpu().numpy())
            else:
                rewards_tr_ep.append(rewards.cpu().numpy())
                
            if args.dr_type=="auto_dr" and pb_ep < pb and not all_dims_covered:
                p=(rewards.sum(0).cpu().numpy()>=thr_r).astype(int).sum()
        ...
        if evaluate and t_eval>eval_freq:
            ...
            for _ in range(eval_eps):
                if args.dr_type:
                    rand_value = args.oracle_rand_value if "oracle" in args.dr_type else np.around(np.random.uniform(low=lower, high=min(lower+eval_step,higher)),3)
                    lower += eval_step
        ...
        #compute & log results
        # compute rewards
        reward_ep = (np.mean(np.stack([np.mean(np.sum(rewards, 0)) for rewards in rewards_tr_ep], 0)))   #sum over T, mean over b, stack horiz one reward per task, mean of tasks
        plot_tr_rewards.append(reward_ep)
        if args.maml:
            reward_val = (np.mean(np.stack([np.mean(np.sum(rewards,0)) for rewards in rewards_val_ep],0)))
        ...
        if eval_rewards_mean>best_reward: 
            ...
            torch.save(policy.state_dict(), f"{models_dir}model{common_name}_seed{seed}"+".pt")
            
        #save best running model [params]
        if running_reward>best_running_reward: 
            best_running_reward=running_reward
            torch.save(policy.state_dict(), f"{models_dir}model_running{common_name}_seed{seed}"+".pt")
    ...
    #%% Results & Plot
    #save results to df
    df = pd.DataFrame(list(zip(plot_tr_rewards,
                               # plot_tr_rewards_std,
                               plot_eval_rewards,
                               # plot_eval_rewards_std,
                               total_timesteps)),
                      columns =['Rewards_Tr', 'Rewards_Eval', 'Total_Timesteps'])
    if args.maml:
        df['Rewards_Val']=plot_val_rewards
        # df['Rewards_Val_Std']=plot_val_rewards_std
    
    if args.dr_type=="active_dr":
        df['Rewards_Disc']=plot_disc_rewards
        # df['Rewards_Disc_Std']=plot_disc_rewards_std
    
    df.to_pickle(f"{plots_tr_dir}results{common_name}_seed{seed}.pkl")
    ...
    # plt.ylabel("Rewards")
    # plt.title(title)
    # plt.savefig(f'{plots_tr_dir}tr{common_name}_seed{seed}.png')
    ...
    #     plt.ylabel("Rewards")
    #     plt.title(title)
    #     plt.savefig(f'{plots_tr_dir}val{common_name}_seed{seed}.png')
    ...
    # plt.ylabel("Rewards")
    # plt.title(title)
    # plt.savefig(f'{plots_tr_dir}ts{common_name}_seed{seed}.png')
    ...
    if "active" in args.dr_type or "auto" in args.dr_type:
        ...
        #     plt.ylabel("Rewards")
        #     plt.title(title)
        #     plt.savefig(f'{plots_tr_dir}disc{common_name}_seed{seed}.png')
        if "auto" in args.dr_type:
            for dim in range(dr):
                ...
                plt.title(title)
                plt.legend()
                plt.savefig(f'{plots_tr_dir}boundaries_change_dim_{dim_name}_{env.rand}{common_name}_seed{seed}.png')
        ...
        df2.to_pickle(f"{plots_tr_dir}sampled_regions{common_name}_seed{seed}.pkl")
