...
if __name__ == '__main__':
    ...
    #%% Evaluation
    if evaluate:
        policy.load_state_dict(torch.load("saved_models/"+env_name+".pt", map_location=device))
        policy.eval()
        tasks = env.sample_tasks(meta_b)
        for task in tasks:
            env.reset_task(task)
            ...
            for episode in episodes:
                ...
                while not done:
                    ...
                    state=torch.from_numpy(o).to(device)
                    ...
                    if adaptation:
                        states=torch.from_numpy(o).float().unsqueeze(0).unsqueeze(0).to(device)
                ...
                eval_rewards.append(np.sum(R))
                if episode % log_ival == 0:
                    log_msg="Rewards Sum: {:.2f}".format(np.sum(R))
                    episodes.set_description(desc=log_msg); episodes.refresh()
